...
#include <linux/fs.h> // Para operaciones de I/O
...
#include <linux/myTrackSyscall.h> //Para encabezado de mi struct y contadores sea accesible
...
#include <linux/myIOStatsSyscall.h> //Estadisticas de un PID       
//--------------------------------------
...
SYSCALL_DEFINE1(capture_memory_snapshot, struct my_memory_snapshot __user *, user_snapshot)
{
	...
	struct sysinfo si; //Estrutura para obtener informacion del sistema
...
// Definición de los contadores globales
atomic_t syscall_open_count = ATOMIC_INIT(0);
atomic_t syscall_write_count = ATOMIC_INIT(0);
atomic_t syscall_read_count = ATOMIC_INIT(0);
atomic_t syscall_fork_count = ATOMIC_INIT(0);

SYSCALL_DEFINE1(track_syscall_usage, struct syscall_usage_stats __user *, user_stats) {
    struct syscall_usage_stats stats;

    // Leer los valores de los contadores globales
    stats.open_count = atomic_read(&syscall_open_count);
    stats.write_count = atomic_read(&syscall_write_count);
    stats.read_count = atomic_read(&syscall_read_count);
    stats.fork_count = atomic_read(&syscall_fork_count);
...
// ESTADISTICAS DE UN PID
SYSCALL_DEFINE2(get_io_throttle, pid_t, pid, struct my_io_stats __user *, io_stats)
{
    struct task_struct *task;
    struct my_io_stats stats;

    // Buscar el proceso por el pid
    task = find_task_by_vpid(pid);
    if (!task) {
        return -ESRCH; //ERROR
    }

    // Obtener la información estadística de I/O del proceso
    stats.bytes_read = task->ioac.read_bytes;
    stats.bytes_written = task->ioac.write_bytes;
    stats.bytes_cancelled = task->ioac.cancelled_write_bytes;
	stats.maj_flt = task->maj_flt;  // Fallos de página mayores
    stats.min_flt = task->min_flt;  // Fallos de página menores

    if (copy_to_user(io_stats, &stats, sizeof(struct my_io_stats))) {
        return -EFAULT;  // Error
    }
